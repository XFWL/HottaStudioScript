...
- **取消热键不工作**：确保以管理员权限运行，检查F12键是否被其他程序占用
...
- **优化钓鱼阶段稳定性**：添加滞后区间（Hysteresis）机制，减少鱼竿频繁左右切换的问题
  - 启动阈值：偏移达到停止阈值的1.5倍才开始按键
  - 停止阈值：偏移小于阈值才释放按键
  - 在两者之间保持当前状态不变，避免鱼微小移动导致的频繁方向切换
- **修复取消提示问题**：钓鱼循环和单独执行各阶段支持F12取消，取消时显示取消提示而非失败提示
- **修复出售渔获/补充鱼饵取消**：重试过程中按F12取消时正确显示取消提示
- **清理无用代码**：删除未使用的按键时间设置相关代码
- **收尾阶段流程重构修复**：
  - 重试识别image0-1.png（最多3次），识别成功则点击后正常结束
  - 重试达上限后识别image0-2.png作为最终判定
  - 收尾阶段点击改为模拟移动（0.2秒）后点击
- **出售渔获/补充鱼饵重试优化**：
  - 识别image1-1.png失败重试时重新按Q键打开背包
  - 识别image2-1-1.png失败重试时重新按R键打开商店
- **下杆阶段持续时间调整**：默认值从10秒调整为15秒
