...
- 识别指定图像并执行相应操作：
  ...
  4. 识别 领取 图片，执行点击操作
- 支持循环执行和重试功能
- **`全局取消热键（F12），如果是选择 一直执行 的话，它会在每轮结束取消整个循环（本人python水平有限，Trae CN不知道为什么只能这样处理）`**
...
- **取消操作**：
  - 按下F12键可以立即取消正在执行的操作
  - 如果是选择 一直执行 的话，它会在每轮结束取消整个循环（本人python水平有限，Trae CN不知道为什么只能这样处理）
  - `全屏情况下，需要先按 Win 键切出菜单，然后快速按 F12`
  - 取消后程序会退出当前执行流程，！！循环情况下会在当前循环轮次结束 取消整个循环，请耐心等待当前轮次执行结束
- **性能影响**：
  - 持续点击操作可能会占用较多系统资源
