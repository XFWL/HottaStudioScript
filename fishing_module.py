...
class FishingModule:
    ...
    def create_ui(self, notebook):
        ...
        self.phase_drop_line_timeout = tk.IntVar(value=30)
        self.phase_fishing_delay = tk.DoubleVar(value=0.25)
        self.phase_cleanup_delay = tk.DoubleVar(value=2)
        self.phase_cleanup_retry_count = tk.IntVar(value=10)
        ...
        ttk.Label(drop_line_frame, text="秒").pack(side=tk.LEFT, padx=2)
        ttk.Label(drop_line_frame, text="持续时间").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(cleanup_frame, text="重试次数").pack(side=tk.LEFT, padx=5)
        ttk.Entry(cleanup_frame, textvariable=self.phase_cleanup_retry_count, width=4).pack(side=tk.LEFT, padx=2)
        ...
        while not self.main_app.cancelled:
            ...
            try:
                ...
                if fish_center and rod_center and fish_bounds:
                    ...
                    self.fishing_log("未检测到有效的鱼或鱼竿位置")
    ...
    def phase_cleanup(self, window, fish_hsv, detect_percent, delay, timeout_triggered=False, retry_count=3):
        ...
        while retry_count < max_retry and not self.main_app.cancelled:
            try:
                ...
                pos = self.find_fishing_template(screenshot, "image0-1.png")
                
                if pos:
                    self.fishing_log(f"识别到image0-1.png，位置: {pos}")
                    ...
                    pyautogui.click(center_x, center_y)
                    pyautogui.click(center_x, center_y)
                    pyautogui.moveTo(center_x, center_y - np.random.randint(50, 76), duration=0.2)
                    self.fishing_log("收尾阶段正常结束")
                    return "success"
                else:
                    self.fishing_log(f"未识别到image0-1.png (重试 {retry_count + 1}/{max_retry})")
                    retry_count += 1
                    time.sleep(1)
        ...
        if self.main_app.cancelled:
            self.fishing_log("收尾阶段被取消")
            return None
        
        self.fishing_log(f"重试次数已达上限({max_retry}次)，尝试识别image0-2.png")
        try:
            window.activate()
            time.sleep(0.02)
            screenshot = self.main_app.capture_window(window)
            ...
            if screenshot is not None:
                pos2 = self.find_fishing_template(screenshot, "image0-2.png")
                if pos2:
                    self.fishing_log(f"识别到image0-2.png，位置: {pos2}")
                    window_left, window_top = window.left, window.top
                    window_width, window_height = window.width, window.height
                    center_x = window_left + window_width // 2
                    center_y = window_top + window_height // 2
            ...
            self.fishing_log("未识别到image0-2.png，收尾阶段失败")
            return "failed"
        
        except Exception as e:
            self.fishing_log(f"识别image0-2.png时发生错误: {str(e)}")
            return "failed"
        ...
        while not self.main_app.cancelled:
            ...
            if self.phase_cleanup_enabled.get():
                result = self.phase_cleanup(
                    ...
                    timeout_triggered,
                    self.phase_cleanup_retry_count.get()
                )
                if result is None:
                    self.fishing_log("收尾阶段被取消")
                    self.main_app.cancelled = False
                    self.main_app.root.after(0, lambda: messagebox.showinfo("操作取消", "收尾阶段已取消"))
                    break
                elif result != "success":
                    self.fishing_log("收尾阶段失败，终止操作")
                    self.main_app.root.after(0, lambda: messagebox.showerror("错误", "收尾阶段失败"))
                    break
    ...
    def execute_cleanup_alone(self):
        ...
        def thread_func():
            ...
            result = self.phase_cleanup(
                window, fish_hsv, self.detect_area_percent.get(),
                self.phase_cleanup_delay.get(),
                retry_count=self.phase_cleanup_retry_count.get()
            )
        ...
        try:
            ...
            while retry_count < 4 and not self.main_app.cancelled:
                ...
                if pos1:
                    ...
                    self.fishing_log(f"未找到image2-1-1.png，重试补充鱼饵 (第{retry_count + 1}/4次)")
                    retry_count += 1
                    if retry_count < 4:
                        pyautogui.press('r')
                        self.fishing_log("按下R键重新打开商店")
                    time.sleep(1)
            
            if pos1 is None:
                self.fishing_log("未找到image2-1.png，达到重试上限")
                return False
            ...
            # 3. 识别image2-1-2.png（最大数量）并保存坐标（最多重试4次）
            pos2 = None
            retry_count = 0
            while retry_count < 4 and not self.main_app.cancelled:
                screenshot = self.main_app.capture_window(window)
                ...
                if pos2:
                    ...
                    self.fishing_log(f"找到image2-1-2.png（最大数量），原始位置: {pos2}，保存位置（上移100px）: {saved_pos1}")
                    window_left, window_top = window.left, window.top
                    ...
                    self.fishing_log(f"未找到image2-1-2.png（最大数量），重试补充鱼饵 (第{retry_count + 1}/4次)")
                    retry_count += 1
                    time.sleep(1)
            
            if pos2 is None:
                self.fishing_log("未找到image2-1-2.png（最大数量），达到重试上限")
                return False
            ...
            # 4. 识别image2-2.png（购买）并点击（最多重试4次）
            pos3 = None
            retry_count = 0
            while retry_count < 4 and not self.main_app.cancelled:
                screenshot = self.main_app.capture_window(window)
                ...
                if pos3:
                    self.fishing_log(f"找到image2-2.png（购买），点击位置: {pos3}")
                    window_left, window_top = window.left, window.top
    ...
    def get_settings(self):
        ...
        return {
            ...
            'phase_cleanup_retry_count': self.phase_cleanup_retry_count.get(),
            'buy_bait_enabled': self.buy_bait_enabled.get(),
            'buy_bait_interval': self.buy_bait_interval.get(),
    ...
    def load_settings(self, settings):
        ...
        self.phase_drop_line_timeout.set(settings.get('phase_drop_line_timeout', 30))
        ...
        self.phase_cleanup_retry_count.set(settings.get('phase_cleanup_retry_count', 10))
